...
int CopyCodeData(uint64_t virtAddr,int perms)
{
		...
		while(curVma != NULL)
		{
			if(curVma->vm_start<= virtAddr && virtAddr < curVma->vm_end)
			{
				...
				else
				{
					...
					{
						printf("\n%s: Process trying to write into a read section. Exitting...",currentTask->memMap->name);
						ExitProcess();
					}
				}
	...
	{
		printf("\n%s: Seg Fault. Process trying to access a section without a corresponding VMA. Exitting...",currentTask->memMap->name);
		ExitProcess();
	}
...
uint64_t HeapStart = 0xF000000;
uint64_t sbrk = 0xF000000;
